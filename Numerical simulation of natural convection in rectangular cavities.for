...
*--------------------------------------------------------------------------
      SUBROUTINE SOLVE
****************************************************************************
	PARAMETER (NX=72,NY=142,NXY=142)
...
***********************************************************************
*                                                                     *
*        Example-4: Numerical simulation                              *
*                                                                     *
***********************************************************************
      DIMENSION T(NX,NY)
      EQUIVALENCE(F(1,1,4),T(1,1))
      DATA RA,PR/1.E3,0.71/
...
C----------------------------------------------------------------------
      ENTRY OUTPUT
        ...
        IF(MOD(ITER,1).EQ.0) THEN
          WRITE(8,403) ITER,T(6,6),SMAX
          WRITE(*,403) ITER,T(6,6),SMAX
        END IF

  400   FORMAT(13X,'ITER',14X,'T(6,6)',12X,'SMAX')
  403   FORMAT(1X,I8,2F20.10)
        ...
        IF(LSTOP.OR.(ITER.EQ.LAST)) THEN
          ...
          WRITE(1,*) 'ZONE I=',L1,',J=',M1-1
          DO J=2,M1
           DO I=1,L1
             WRITE(1,*) XU(I),Y(J),V(I,J)
            END DO
          END DO
          CLOSE(1)
		   !输出流线SF
      ...
      ENTRY GAMSOR
        ...
        IF(ISOURCE.EQ.1) THEN
          ...
          IF (NF.EQ.2) THEN
			DO I=1,L1
				DO J=1,M1
				GAM(I,J)=PR                !v方程无量纲化后的gama
				CON(I,J)=RA*PR*T(I,J)      !u方程无量纲化后的源项
				END DO
			END DO
          END IF
